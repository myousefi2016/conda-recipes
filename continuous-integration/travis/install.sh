...
if [ $TRAVIS_OS_NAME == "linux" ]; then sudo apt-get update && sudo apt-get install -yqq git wget \
                                        build-essential libxt-dev libgl1-mesa-glx libgl1-mesa-dev libglapi-mesa libosmesa-dev \
                                        libglu-dev libglw1-mesa elfutils gamin libglw1-mesa-dev mesa-utils xfstt ttf-liberation \
                                        ttf-mscorefonts-installer xfonts-100dpi xfonts-75dpi; fi
