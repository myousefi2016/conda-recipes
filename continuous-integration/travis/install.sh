...
if [ $TRAVIS_OS_NAME == "linux" ]; then sudo apt-get update && sudo apt-get install -yqq git wget libegl1-mesa libegl1-mesa-drivers libxt-dev libgl1-mesa-glx libgl1-mesa-dev libglapi-mesa libosmesa-dev libgl1-mesa-dri libXt-devel mesa-libGLU-devel build-essential; fi
